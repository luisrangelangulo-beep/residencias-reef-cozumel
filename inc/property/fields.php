...
function lvc_register_property_fields() {
	...
	$fields = apply_filters( 'lvc_property_fields', array(
		...
		array( 'key' => 'field_lvc_hero_image', 'label' => 'Hero Image (Property Page)', 'name' => 'hero_image', 'type' => 'url', 'instructions' => 'Optional wide image used as the first choice for the property-page hero. Recommended minimum width: 1600px. When empty, the theme selects the best safe fallback.' ),
		array( 'key' => 'field_lvc_feature_image', 'label' => 'Feature Image (Cards/Grid)', 'name' => 'feature_image', 'type' => 'url', 'instructions' => 'Primary crop for homepage, archive, area, collection, and related-property cards. Recommended ratio: 4:3. It is only a fallback for the property hero when no dedicated hero is available.' ),
		array( 'key' => 'field_lvc_gallery', 'label' => 'Gallery Preview URLs (Optional)', 'name' => 'gallery_squares', 'type' => 'textarea', 'instructions' => 'Optional curated preview of up to six images, one URL per line. Leave empty to use the first six images from Full Gallery URLs automatically. Legacy values remain supported.' ),
...
/*
 * This site still has a database-managed legacy field group containing
 * gallery_slider. Clarify the two controls in wp-admin without renaming or
 * deleting either field, which would orphan existing property media.
 */
add_filter( 'acf/prepare_field/name=gallery_slider', function ( $field ) {
	$field['label']        = 'Full Gallery URLs (Primary)';
	$field['instructions'] = 'The complete ordered property gallery, one image URL per line. This is the primary source used by the full slider, schema, and photo count.';
	return $field;
} );

add_filter( 'acf/prepare_field/name=gallery_squares', function ( $field ) {
	$field['label']        = 'Gallery Preview URLs (Optional)';
	$field['instructions'] = 'Optional curated preview of up to six images. Leave empty and the theme will use the first six Full Gallery images automatically. Existing legacy values remain supported.';
	return $field;
} );
