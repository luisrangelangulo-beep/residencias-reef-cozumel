...
// Read the canonical field schema first, then the site's legacy aliases.
$h1_title             = lvc_field( 'h1_title', $villa_id, get_the_title() );
$display_title_parts  = preg_split( '/\s+(?:—|\||-)\s+/u', html_entity_decode( (string) $h1_title, ENT_QUOTES, 'UTF-8' ), 2 );
$display_title        = trim( (string) ( $display_title_parts[0] ?? $h1_title ) );
$bedrooms             = lvc_field( 'bed_count', $villa_id );
$bathrooms            = lvc_field( 'bath_count', $villa_id );
$max_guests           = lvc_field( 'guests_max', $villa_id );
$view_type            = get_field( 'view_type' );
$community            = get_field( 'community' );
$property_description = lvc_field( 'property_descr', $villa_id );
$indoor_living        = get_field( 'indoor_living' );
$outdoor_living       = get_field( 'outdoor_living' );
$bedroom_description  = lvc_field( 'bedroom_desc', $villa_id );
...
/*
 * gallery_slider is the primary ordered gallery. gallery_squares is retained
 * as an optional curated preview and a lossless legacy fallback. Merging here
 * means an old property cannot lose images because its fields are not yet
 * migrated.
 */
$all_gallery_urls     = array_values( array_unique( array_merge( $gallery_slider_urls, $gallery_square_urls ) ) );
$gallery_preview_urls = $gallery_square_urls ? array_slice( $gallery_square_urls, 0, 6 ) : array_slice( $all_gallery_urls, 0, 6 );
...
$schema = array(
	...
	'name'        => html_entity_decode( wp_strip_all_tags( $h1_title ), ENT_QUOTES, 'UTF-8' ),
	'url'         => $property_url,
	'image'       => $schema_images,
	'description' => html_entity_decode( wp_strip_all_tags( $property_description ? $property_description : $intro_quote ), ENT_QUOTES, 'UTF-8' ),
...
<style>
	...
	.rrc-location{background:var(--lvc-accent);color:#070c11}.rrc-location__grid{display:grid;grid-template-columns:minmax(0,.75fr) minmax(0,1fr);gap:clamp(2rem,5vw,4rem);align-items:start}.rrc-location .rrc-eyebrow{color:rgba(4,7,10,.62)}.rrc-location h2{margin:0;font-family:var(--lvc-font-display);font-size:clamp(2rem,4vw,3.4rem);font-weight:400;letter-spacing:-.05em}.rrc-location p{margin:1rem 0 0;line-height:1.75;color:rgba(4,7,10,.78)}.rrc-location__facts{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1px;background:rgba(4,7,10,.18);border:1px solid rgba(4,7,10,.18)}.rrc-location__fact{background:rgba(255,255,255,.24);padding:1rem}.rrc-location__fact span{display:block;color:rgba(4,7,10,.58);font-size:.62rem;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.rrc-location__fact strong{display:block;margin-top:.35rem;color:#070c11}
	...
	/* Media architecture and interaction refinements. */
	.rrc-hero{overflow:hidden}.rrc-hero__bg{width:100%;height:100%;object-fit:cover;object-position:center;background:none}
	.rrc-square-grid figure{scroll-snap-align:start}.rrc-square-grid img{transition:transform .45s ease}.rrc-square-grid figure:hover img{transform:scale(1.035)}
	.rrc-gallery-jump{display:inline-flex;margin-top:1.5rem;padding-bottom:.2rem;border-bottom:1px solid currentColor;color:var(--lvc-accent);font-size:.72rem;font-weight:800;letter-spacing:.12em;text-decoration:none;text-transform:uppercase}
	.rrc-assist{padding:clamp(1.5rem,3vw,2.25rem) 0;background:linear-gradient(105deg,rgba(235,139,78,.12),rgba(16,21,28,.98) 45%);border-color:rgba(235,139,78,.22)}.rrc-assist__inner{width:min(1120px,calc(100% - clamp(2rem,6vw,6rem)));padding:1.25rem 1.4rem;border:1px solid var(--lvc-border);background:rgba(10,12,15,.42)}.rrc-assist h2{font-size:clamp(1.35rem,2vw,1.75rem);font-weight:400}.rrc-assist p{max-width:620px}
	.rrc-photo-grid .rrc-title{max-width:980px;font-size:clamp(1.85rem,3vw,3rem);line-height:1.12}.rrc-photo-grid__grid{grid-auto-columns:min(78vw,840px)}.rrc-photo-grid figure{height:auto;aspect-ratio:16/10;max-height:540px;display:flex;align-items:center;justify-content:center;scroll-snap-stop:always}.rrc-photo-grid img{object-fit:scale-down;background:var(--lvc-card)}.rrc-gallery-arrow{transition:background .2s ease,border-color .2s ease,color .2s ease,opacity .2s ease}.rrc-gallery-arrow:disabled{opacity:.3;cursor:default}
	.rrc-gallery-meta{display:flex;align-items:center;justify-content:space-between;gap:1rem;margin-top:.65rem}.rrc-gallery-meta .rrc-gallery-hint{margin:0}.rrc-gallery-counter{margin:0;color:var(--lvc-accent);font-size:.76rem;font-variant-numeric:tabular-nums;letter-spacing:.06em;text-transform:uppercase}
	@media(max-width:680px){.rrc-square-grid{display:grid;grid-template-columns:none;grid-auto-flow:column;grid-auto-columns:82vw;gap:.8rem;overflow-x:auto;overscroll-behavior-x:contain;scroll-snap-type:x mandatory;padding-bottom:.75rem;scrollbar-width:thin}.rrc-square-grid figure:hover img{transform:none}.rrc-photo-grid__grid{grid-auto-columns:88vw}.rrc-photo-grid figure{height:auto;aspect-ratio:4/3;max-height:440px}.rrc-gallery-meta{align-items:flex-start}.rrc-gallery-hint{max-width:72%}.rrc-assist__inner{display:grid;padding:1.2rem}.rrc-assist__actions{display:grid;grid-template-columns:1fr}}
	@media(prefers-reduced-motion:reduce){.rrc-square-grid img,.rrc-gallery-arrow{transition:none}.rrc-photo-grid__grid,.rrc-square-grid{scroll-behavior:auto}}
</style>
...
<main class="rrc-villa">
	<section class="rrc-hero" aria-label="<?php echo esc_attr( $h1_title ); ?>">
		...
		<div class="rrc-container rrc-hero__grid">
			<div class="rrc-hero__copy">
				...
				<p class="rrc-hero__rate">Rates on Request</p>
	...
	<?php if ( ! empty( $gallery_preview_urls ) ) : ?><section class="rrc-square-gallery" aria-label="Villa photo preview"><div class="rrc-container" style="text-align:center;"><span class="rrc-eyebrow">Photo Tour</span><h2 class="rrc-title">Explore the <em>spaces</em></h2><div class="rrc-square-grid"><?php foreach ( $gallery_preview_urls as $index => $image_url ) : ?><figure><img src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $h1_title . ( $location_line ? ' in ' . $location_line : '' ) . ' — preview photo ' . ( $index + 1 ) ); ?>" loading="lazy" decoding="async"></figure><?php endforeach; ?></div><?php if ( $photo_count > count( $gallery_preview_urls ) ) : ?><a class="rrc-gallery-jump" href="#gallery">View all <?php echo esc_html( $photo_count ); ?> photos &darr;</a><?php endif; ?></div></section><?php endif; ?>
	...
	<section class="rrc-assist" aria-label="Booking assistance"><div class="rrc-container rrc-assist__inner"><div><h2>Need help deciding if this villa fits?</h2><p>Share your dates, group profile, and priorities. We will confirm availability, explain the trade-offs, and suggest stronger alternatives if this is not the best match.</p></div><div class="rrc-assist__actions"><a href="#inquiry" class="rrc-btn rrc-btn--primary">Check Your Dates</a><?php if ( $whatsapp_url ) : ?><a href="<?php echo esc_url( $whatsapp_url ); ?>" target="_blank" rel="noopener noreferrer" class="rrc-btn rrc-btn--outline">Talk to Us</a><?php endif; ?></div></div></section>

	<?php if ( ! empty( $all_gallery_urls ) ) : ?><section class="rrc-photo-grid" id="gallery" aria-label="Full villa photo gallery"><div class="rrc-container"><span class="rrc-eyebrow">Full Gallery</span><h2 class="rrc-title">A closer look at <em><?php echo esc_html( $display_title ); ?></em></h2><div class="rrc-photo-slider"><button class="rrc-gallery-arrow rrc-gallery-arrow--prev" type="button" aria-label="Previous gallery photo" data-rrc-gallery-prev>&lsaquo;</button><div class="rrc-photo-grid__grid" data-rrc-gallery-track tabindex="0" aria-label="<?php echo esc_attr( $h1_title . ' photo gallery' ); ?>"><?php foreach ( $all_gallery_urls as $index => $image_url ) : ?><figure data-rrc-gallery-slide><img src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $h1_title . ( $location_line ? ' in ' . $location_line : '' ) . ' — villa photo ' . ( $index + 1 ) ); ?>" loading="lazy" decoding="async"></figure><?php endforeach; ?></div><button class="rrc-gallery-arrow rrc-gallery-arrow--next" type="button" aria-label="Next gallery photo" data-rrc-gallery-next>&rsaquo;</button></div><div class="rrc-gallery-meta"><p class="rrc-gallery-hint">Use the arrows, keyboard, or swipe to view more photos.</p><p class="rrc-gallery-counter" aria-live="polite"><span data-rrc-gallery-current>1</span> / <?php echo esc_html( $photo_count ); ?></p></div></div></section><?php endif; ?>
...
<script>
(function(){var sticky=document.getElementById('rrcSticky');var inquiry=document.getElementById('inquiry');var toggleSticky=function(){if(!sticky){return;}var y=window.scrollY||window.pageYOffset||0;sticky.classList.toggle('is-visible',y>700&&(!inquiry||inquiry.getBoundingClientRect().top>250));};window.addEventListener('scroll',toggleSticky,{passive:true});toggleSticky();var checkinInput=document.querySelector('input[name="checkin"]');var checkoutInput=document.querySelector('input[name="checkout"]');if(checkinInput&&checkoutInput){checkinInput.addEventListener('change',function(){if(this.value){checkoutInput.min=this.value;if(checkoutInput.value&&checkoutInput.value<=this.value){checkoutInput.value='';}}});}var galleryTrack=document.querySelector('[data-rrc-gallery-track]');var galleryPrev=document.querySelector('[data-rrc-gallery-prev]');var galleryNext=document.querySelector('[data-rrc-gallery-next]');var galleryCurrent=document.querySelector('[data-rrc-gallery-current]');if(galleryTrack&&galleryPrev&&galleryNext){var gallerySlides=Array.prototype.slice.call(galleryTrack.querySelectorAll('[data-rrc-gallery-slide]'));var scrollGallery=function(direction){var first=gallerySlides[0];var distance=first?first.getBoundingClientRect().width+16:Math.max(galleryTrack.clientWidth*.72,420);galleryTrack.scrollBy({left:direction*distance,behavior:'smooth'});};var updateGallery=function(){if(!gallerySlides.length){return;}var target=galleryTrack.scrollLeft+galleryTrack.clientWidth*.5;var current=0;var best=Infinity;gallerySlides.forEach(function(slide,index){var center=slide.offsetLeft+slide.offsetWidth*.5;var delta=Math.abs(center-target);if(delta<best){best=delta;current=index;}});if(galleryCurrent){galleryCurrent.textContent=String(current+1);}galleryPrev.disabled=current===0;galleryNext.disabled=current===gallerySlides.length-1;};galleryPrev.addEventListener('click',function(){scrollGallery(-1);});galleryNext.addEventListener('click',function(){scrollGallery(1);});galleryTrack.addEventListener('keydown',function(event){if(event.key==='ArrowLeft'){event.preventDefault();scrollGallery(-1);}if(event.key==='ArrowRight'){event.preventDefault();scrollGallery(1);}});var galleryTick=false;galleryTrack.addEventListener('scroll',function(){if(!galleryTick){galleryTick=true;window.requestAnimationFrame(function(){updateGallery();galleryTick=false;});}},{passive:true});window.addEventListener('resize',updateGallery,{passive:true});updateGallery();}})();
</script>
