...
$area_term  = ( $area_terms && ! is_wp_error( $area_terms ) ) ? $area_terms[0] : null;
...
<style>
	...
	@media(max-width:1050px){.rrc-hero__grid,.rrc-fit__box,.rrc-overview__grid,.rrc-location__grid{grid-template-columns:1fr}.rrc-snapshot,.rrc-detail-card{position:static;max-width:620px}.rrc-related__grid{grid-template-columns:repeat(2,1fr)}.rrc-fit__panel{border-left:0;padding-left:0}.rrc-space-grid{grid-template-columns:1fr 1fr}}@media(max-width:680px){.rrc-container,.rrc-narrow{width:calc(100% - 2rem)}.rrc-hero{min-height:auto;padding-top:7rem}.rrc-photo-link{position:static;display:inline-flex;margin:1rem 0 0 1rem}.rrc-hero h1{font-size:clamp(2.25rem,11vw,3.2rem)}.rrc-stats,.rrc-space-grid,.rrc-included__grid,.rrc-location__facts,.rrc-related__grid{grid-template-columns:1fr}.rrc-sticky__inner{flex-direction:column;align-items:stretch}.rrc-hero__actions,.rrc-fit__actions,.rrc-assist__actions,.rrc-sticky__actions{display:grid;grid-template-columns:1fr}.rrc-sticky__info{display:none}.rrc-photo-grid__grid{grid-auto-columns:88vw}.rrc-photo-grid figure{height:clamp(320px,72vw,520px)}.rrc-gallery-arrow{width:42px;height:42px;font-size:1.35rem}.rrc-gallery-arrow--prev{left:.5rem}.rrc-gallery-arrow--next{right:.5rem}}
</style>
...
<main class="rrc-villa">
	<section class="rrc-hero" aria-label="<?php echo esc_attr( $h1_title ); ?>">
		...
		<div class="rrc-container rrc-hero__grid">
			<div class="rrc-hero__copy">
				<div class="rrc-kicker"><span><?php echo esc_html( $h1_title ); ?></span><?php if ( $location_line ) : ?><span><?php echo esc_html( $location_line ); ?></span><?php endif; ?></div>
	...
	<?php if ( ! empty( $gallery_square_urls ) ) : ?><section class="rrc-photo-grid" aria-label="Villa photo preview"><div class="rrc-container"><span class="rrc-eyebrow">Photo Tour</span><h2 class="rrc-title">Explore the <em>spaces</em></h2><div class="rrc-gallery" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:8px;"><?php foreach ( $gallery_square_urls as $index => $image_url ) : ?><figure style="margin:0;aspect-ratio:1;overflow:hidden;"><img src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $h1_title . ' square photo ' . ( $index + 1 ) ); ?>" style="width:100%;height:100%;object-fit:cover;" loading="lazy" decoding="async"></figure><?php endforeach; ?></div></div></section><?php endif; ?>
